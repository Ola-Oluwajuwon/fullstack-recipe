...
// Transform TransformedMeal to Recipe interface
const transformToRecipe = (transformedMeal: any): Recipe => {
  return {
    id: transformedMeal.id,
    title: transformedMeal.title,
    image: transformedMeal.image,
    description: transformedMeal.description,
    cookTime: transformedMeal.cookTime,
    servings: transformedMeal.servings,
  };
};

const SearchRecipeScreen = (): React.JSX.Element => {
  ...
  const performSearch = async (query: string): Promise<Recipe[]> => {
    // if no search query
    if (!query.trim()) {
      const randomMeals = await MealAPI.getMultipleRandomMeals(12);
      return randomMeals
        .map((meal: any) => MealAPI.transformMealData(meal))
        .filter((meal): meal is any => meal !== null)
        .map(transformToRecipe);
    }
